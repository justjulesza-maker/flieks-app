 ...
 *
 * Started by flieks-film-live when an admin puts a film live. Needs the
...
exports.handler = async event => {
  ...
  let filmId = '';
  try { filmId = String(JSON.parse(event.body || '{}').filmId || ''); } catch {}
  try {
    const r = await notifyWatchers(filmId);
