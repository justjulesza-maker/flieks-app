 ...
 * admin sending news about a film ('update'). 'followers' emails a filmmaker's
 * followers about a new film (event 'soon' or 'live'; 'live' also runs after the
 * watchlist email). Needs the
...
const { notifyFollowers } = require('../lib/makers');
...
exports.handler = async event => {
  ...
  let filmId = '', kind = 'live', updateId = null, ev = null;
  try { const b = JSON.parse(event.body || '{}'); filmId = String(b.filmId || ''); kind = b.kind || 'live'; updateId = b.updateId || null; ev = b.event || null; } catch {}
  try {
    if (kind === 'followers') {
      console.log('[watch-notify] followers', filmId, JSON.stringify(await notifyFollowers(filmId, ev)));
    } else {
      const r = await notifyWatchers(filmId, { kind, updateId });
      console.log('[watch-notify]', filmId, JSON.stringify(r));
      // Then the filmmaker's followers, skipping anyone who just had the watchlist email.
      if (kind === 'live') console.log('[watch-notify] followers', filmId, JSON.stringify(await notifyFollowers(filmId, 'live')));
    }
  } catch (e) {
